...
async function getAccessToken(env){
  ...
  const j=await r.json(); if(!r.ok) throw new Error("oauth_failed:"+JSON.stringify(j)); return j.access_token;
}
export async function onRequestPost({request,env}){
  try{
    const {seed}=await request.json(); if(!seed) return new Response(JSON.stringify({error:"Seed required"}),{status:400});
